...
const FOLDER_ID = "1Q3f19W4QtO7mKsCNRfMEV5I1GrEEroo9";
...
function doPost(e) {
  ...
  try {
    ...
    try {
      const summaryName = "ריכוז נרשמים שכבת ה";
...
function buildPdfBlob(data, sigBlob) {
  ...
  const title = body.appendParagraph(`${rlm}טופס אישור השתתפות במסיבת סיום שכבת כיתות ה'${rlm}`)
  ...
  const intro = body.appendParagraph(
    `${rlm}הורים ותלמידים יקרים, לקראת מסיבת הסיום השנתית של שכבת כיתות ה\' (כיתות ה-1, ה-2) ` +
    `מבית ספר "ערמונים", אשר תתקיים בתאריך 11/6/2026, בין השעות 16:00-19:00, במתחם בריכת כפר מעש, ` +
  ...
  const items = [
    ...
    "מובהר ומוסכם בזאת כי אין ולא תהיה כל אחריות במישרין ו/או בעקיפין להורי ועד כיתות ה' בבי״ס ערמונים לכל נזק (גוף, נפש או רכוש), פציעה, אובדן או פגיעה שיגרמו לילד/ה או לרכושו/ה במהלך האירוע, בדרך אליו או בחזרה ממנו.",
    ...
    "מובהר ומוסכם בזאת, כי ההורים מוותרים ויתור סופי, מוחלט, מלא ובלתי חוזר על כל טענה, דרישה, דרישת פיצוי, קובלנה ו/או תביעה משפטית או אחרת, מכל סיבה שהיא, כנגד הורי ועד כיתות ה' 1-2 ו/או כנגד מי מטעמם.",
